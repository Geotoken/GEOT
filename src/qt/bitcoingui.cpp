...
{
    resize(850, 550);
    setWindowTitle(tr("Geotoken") + " - " + tr("Wallet"));
#ifndef Q_WS_MAC
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Geotoken client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Geotoken client"));
...
void BitcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
    ...
    {
        // Click on system tray icon triggers "show/hide Geotoken"
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Geotoken network", "", count));
}
...
void BitcoinGUI::setMining(bool mining, int hashrate)
{
    if (mining)
    {
        ...
        labelMiningIcon->setToolTip(tr("Mining Geotoken at %1 hashes per second").arg(hashrate));
    }
    else
    {
        labelMiningIcon->setPixmap(QIcon(":/icons/mining_inactive").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
        labelMiningIcon->setToolTip(tr("Not mining Geotoken"));
    }
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Geotoken address or malformed URI parameters."));
    }
